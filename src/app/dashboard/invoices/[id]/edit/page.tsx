...
import { ArrowLeft, Save, Plus, Trash2, Building } from "lucide-react"
import { toast } from "sonner"
import type { Client, Company, InvoiceItem, InvoiceFormData, Invoice } from "@/types/invoice"
...
export default function EditInvoicePage() {
  ...
  const updateItem = (id: string, field: keyof InvoiceItem, value: string | number) => {
    setItems(prev =>
      prev.map(item =>
        item.id === id
          ? { ...item, [field]: value, amount: field === 'quantity' || field === 'unitPrice' ? (field === 'quantity' ? Number(value) * item.unitPrice : item.quantity * Number(value)) : item.amount }
          : item
      )
    )
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate form
    if (!formData.invoiceNumber || !formData.clientId || items.some(item => !item.description || item.quantity <= 0 || item.unitPrice <= 0)) {
      toast.error("Please fill in all required fields and ensure all items have valid quantities and prices")
      setIsLoading(false)
      return
    }
    ...
    try {
      const response = await fetch(`/api/invoices/${invoiceId}`, {
        ...
        body: JSON.stringify({
          ...
          items: items.map(({ id, ...item }) => item) // Remove id field for API
        }),
      })
